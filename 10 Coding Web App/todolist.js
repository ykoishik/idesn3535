window.Todo = {
  ...
  setup: function() {
    Todo.addGoalButton.addEventListener('click', Todo.onAddTaskClicked);
    Todo.container.addEventListener('click', Todo.onTodolistClicked);

    Todo.renderTasks();
  },
  onAddTaskClicked: function(event) {
    var goalName = Todo.newTaskInput.value;
    Todo.newTaskInput.value = "";

    var goalHTML = Todo.template.replace("<!-- TASK_NAME -->", goialName);
